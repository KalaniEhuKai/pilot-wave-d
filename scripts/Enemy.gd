...
## Enemy.gd - Cyberpunk shmup enemy craft (Scout & Heavy Bomber) with formation wipe tracking.
...
var main_color: Color = Color(1.0, 0.2, 0.4, 1.0) # Neon Crimson / Magenta
var accent_color: Color = Color(1.0, 0.6, 0.1, 1.0) # Bright Amber
...
func setup(p_type: EnemyType, p_pos: Vector2, p_squad_id: int, p_spawner: Node) -> void:
	enemy_type = p_type
...
func _physics_process(delta: float) -> void:
	...
	var move_dir = (-GameAxis.forward).normalized()
	rotation = move_dir.angle()
	
	# Weapon firing
	_handle_firing(delta)
	
	# Boundary check - if enemy flew past the player and escaped
...
func _draw() -> void:
	...
	if enemy_type == EnemyType.SCOUT:
		# Sharp, aggressive needle fighter (facing Vector2.RIGHT)
		...
		# Heavy armored bomber hexagon craft
